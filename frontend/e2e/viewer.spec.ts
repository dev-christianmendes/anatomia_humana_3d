...
test('desktop and mobile render a framed, interactive model', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Restaurar visualização' }).click()
  await expect.poll(async () => (await scenePixels(page)).checksum).toBe(initial.checksum)
  ...
  await page.mouse.move(bounds.x + bounds.width / 2 + 60, bounds.y + bounds.height / 2 + 10, { steps: 6 })
  await page.mouse.up()
