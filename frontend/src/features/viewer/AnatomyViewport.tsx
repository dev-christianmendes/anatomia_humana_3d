...
function Model({ wireframe, onLoaded }: Pick<ViewerProps, 'wireframe' | 'onLoaded'>) {
  ...
  const [model] = useState(() => {
    ...
    copy.traverse((object) => {
      if (object instanceof Mesh) {
        object.material = new MeshStandardMaterial({ color: BASE_COLOR, roughness: 0.55 })
      }
    })
  ...
  useEffect(() => {
    ...
    invalidate()
  }, [materialsByStructure, selected, hovered, isolated, invalidate])
...
export default function AnatomyViewport(props: ViewerProps) {
  ...
  return <ViewerBoundary>
    <Canvas className="viewport-canvas"
      ...
      <Suspense fallback={<Html center><div className="loading" role="status">Carregando modelo…</div></Html>}>
        <Model wireframe={props.wireframe} onLoaded={props.onLoaded} />
      </Suspense>
